...
public static class AuthExtensions
{
    ...
    /// <summary>Token issuer embedded in all self-issued JWTs. Public for validation by trusted consumers (e.g. Web host).</summary>
    public const string TokenIssuer = "Technoart";

    /// <summary>Token audience embedded in all self-issued JWTs. Public for validation by trusted consumers (e.g. Web host).</summary>
    public const string TokenAudience = "Technoart.API";
        ...
        this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
            ...
            {
                // .NET 10 changed MapInboundClaims default to false.
                // Our code uses ClaimTypes.* (long URIs) everywhere, so we need the mapping
                // from short JWT names (sub, email) back to long .NET claim type URIs.
    ...
    /// <summary>
    /// Builds <see cref="TokenValidationParameters"/> for validating self-issued JWTs
    /// generated by <see cref="GenerateApiToken"/>. Use this on any host that needs to
    /// verify a token without setting up full JWT Bearer middleware (e.g. the Web host
    /// validating a token returned from the API during <c>OnTicketReceived</c>).
    /// </summary>
    public static TokenValidationParameters CreateTokenValidationParameters(string signingKey) =>
        new()
        {
            ValidateIssuer = true,
            ValidIssuer = TokenIssuer,
